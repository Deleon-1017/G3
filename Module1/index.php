...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // FIXED: Changed to float for decimal values
  ...
  try {
    if ($id) {
      // FIXED: Added unit_price to UPDATE query
      ...
      // FIXED: Added unit_price to INSERT query
...
<body>
        ...
        <tbody>
          <?php foreach ($items as $it): ?>
            <tr>
              ...
              <!-- FIXED: Properly display unit_price as currency -->
          ...
          <?php endforeach;
          if (!count($items)): ?>
            <!-- FIXED: Updated colspan to match new column count -->
            <tr>
              <td colspan="10" class="small">No items found.</td>
            </tr>
          <?php endif; ?>
        </tbody>
      </table>
    ...
    <div class="modal-content">
      ...
      <form method="post" id="itemForm">
        ...
        <div class="form-row">
          ...
          <!-- FIXED: Corrected ID and added proper attributes -->
          <div class="col"><label>Unit Price (₱)<input class="input" name="unit_price" id="item_unit_price" type="number" step="0.01" min="0" value="0"></label></div>
        </div>
        <div class="form-row">
  ...
  <script>
    ...
    function openEditModal(item) {
      ...
      // FIXED: Corrected ID and formatted as float
      document.getElementById('item_unit_price').value = parseFloat(item.unit_price || 0).toFixed(2);
